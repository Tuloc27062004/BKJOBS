...
import { DEMO_GENDERS, DEMO_MODE, DEMO_STATUS_LOOKUPS } from '@/lib/demo';
...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchStatuses = async () => {
      if (DEMO_MODE) {
        setStatuses(DEMO_STATUS_LOOKUPS);
        return;
      }
      try {
        const token = await getAccessToken();
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (DEMO_MODE) {
      toast({
        title: 'Thành công',
        description: 'Đã cập nhật ảnh đại diện demo',
      });
  ...
  const handleCVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadingCV(true);
    if (DEMO_MODE) {
      await updateProfile({ cv: file.name, cv_filename: file.name });
      toast({
        title: 'Thành công',
        description: 'Đã tải lên CV demo',
      });
  ...
  const handleDeleteCV = async () => {
    if (DEMO_MODE) {
      await updateProfile({ cv: undefined, cv_filename: undefined });
      toast({
        title: 'Thành công',
        description: 'Đã xóa CV demo',
      });
      return;
    }
    try {
