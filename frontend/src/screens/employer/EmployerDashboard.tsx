...
const EmployerDashboard = () => {
  ...
  useEffect(() => {
    if (!user) return;
    if (DEMO_MODE) {
      const myJobs = (DEMO_JOB_FORMS as JobForm[]).filter((j) =>
        j.created_by === user.username || j.created_by === user.email || j.created_by === 'demo_employer'
      );
      setJobs(myJobs);
      (async () => {
        ...
        setUserStatuses(Object.fromEntries(allApps.map((a) => [a.userId, 'ACTIVE'])));
        setLoading(false);
      })();
      return;
    }
  ...
  const handleDeleteJob = async (jobId: number) => {
    if (confirm('Bạn có chắc chắn muốn xóa tin tuyển dụng này?')) {
      if (DEMO_MODE) {
        ...
        toast({ title: 'Đã xóa', description: 'Tin tuyển dụng demo đã được xóa' });
        return;
      }
      try {
