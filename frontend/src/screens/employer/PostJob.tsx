...
import { DEMO_COMPANIES, DEMO_CURRENCIES, DEMO_DISTRICTS, DEMO_JOB_TYPES, DEMO_MODE, DEMO_PROVINCES, DEMO_WARDS, DEMO_WORK_FORMATS } from '@/lib/demo';
...
const PostJob = () => {
  ...
  useEffect(() => {
    if (DEMO_MODE) {
      setWorkFormats(DEMO_WORK_FORMATS);
      setJobTypes(DEMO_JOB_TYPES);
      setCurrencies(DEMO_CURRENCIES);
      setVerifiedCompanies(DEMO_COMPANIES);
      return;
    }
    (async () => {
      try {
  ...
  useEffect(() => {
    if (DEMO_MODE) {
      setProvinces(DEMO_PROVINCES as LocationItem[]);
      return;
    }
    (async () => {
      try {
  ...
  useEffect(() => {
    if (DEMO_MODE) {
      const demoDistricts = ((DEMO_DISTRICTS as Record<string, readonly { id: string; name: string }[]>)[formData.province] || []).map((item) => ({
        ...item,
        code: item.id,
      }));
      setDistricts(demoDistricts as LocationItem[]);
  ...
  useEffect(() => {
    if (DEMO_MODE) {
      const demoWards = ((DEMO_WARDS as Record<string, readonly { id: string; name: string }[]>)[formData.district] || []).map((item) => ({
        ...item,
        code: item.id,
      }));
      setWards(demoWards as LocationItem[]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (DEMO_MODE) {
        toast({ title: 'Đăng tin thành công!', description: 'Bài đăng demo đã được tạo.' });
