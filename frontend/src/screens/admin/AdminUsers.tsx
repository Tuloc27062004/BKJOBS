...
import { DEMO_MODE, DEMO_STATUS_LOOKUPS, DEMO_USER_LIST } from '@/lib/demo';
...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    if (DEMO_MODE) {
      setAllUsers(DEMO_USER_LIST as User[]);
      return;
    }
    try {
  ...
  const fetchStatuses = async () => {
    if (DEMO_MODE) {
      setStatuses(DEMO_STATUS_LOOKUPS as StatusLookup[]);
      return;
    }
    try {
      const token = await getAccessToken();
  ...
  const handleSetStatus = async (userId: number, statusCode: string) => {
    if (DEMO_MODE) {
      setAllUsers(prev => prev.map(u => u.id === userId ? { ...u, status: statusCode } : u));
      toast({ title: 'Thành công', description: 'Đã cập nhật trạng thái demo' });
      return;
    }
    try {
      const token = await getAccessToken();
  ...
  const handleApproveAllUsers = async () => {
    ...
    if (DEMO_MODE) {
      setAllUsers(prev => prev.map(u => u.status?.toUpperCase() === 'PENDING_VERIFICATION' ? { ...u, status: 'ACTIVE' } : u));
      toast({ title: "Đã duyệt toàn bộ", description: `Đã duyệt ${pendingVerification.length}/${pendingVerification.length} người dùng` });
      return;
    }
    try {
      const token = await getAccessToken();
