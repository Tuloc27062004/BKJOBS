...
import { DEMO_COMPANIES, DEMO_CURRENCIES, DEMO_GENDERS, DEMO_JOB_TYPES, DEMO_MODE, DEMO_STATUS_LOOKUPS, DEMO_WORK_FORMATS } from '@/lib/demo';
...
const AdminLookups = () => {
  ...
  const fetchCategory = async (category: LookupCategory) => {
    ...
    if (DEMO_MODE) {
      const demoData: Record<string, LookupItem[]> = {
        'verified-companies': DEMO_COMPANIES.map((item, index) => ({ ...item, is_active: true, order: index + 1 })),
        'work-formats': DEMO_WORK_FORMATS.map((item, index) => ({ ...item, is_active: true, order: index + 1 })),
        'job-types': DEMO_JOB_TYPES.map((item, index) => ({ ...item, is_active: true, order: index + 1 })),
        'currencies': DEMO_CURRENCIES.map((item, index) => ({ ...item, is_active: true, order: index + 1 })),
        'roles': [
          { code: 'ADMIN', name: 'Quản trị viên', is_active: true, order: 1 },
          { code: 'USER', name: 'Người dùng', is_active: true, order: 2 },
        ],
        'genders': DEMO_GENDERS.map((item, index) => ({ ...item, is_active: true, order: index + 1 })),
        'statuses': DEMO_STATUS_LOOKUPS.map((item, index) => ({ ...item, is_active: true, order: index + 1 })),
      };
      setLookupData(prev => ({ ...prev, [category.key]: demoData[category.key] || [] }));
