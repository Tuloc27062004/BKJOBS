...
const AdminJobs = () => {
  ...
  const fetchJobs = async () => {
    if (DEMO_MODE) {
      setJobs((DEMO_JOB_FORMS as JobForm[]).filter((j) => j.is_active !== false));
      return;
    }
    try {
  ...
  const handleApproveAll = async () => {
    ...
    if (DEMO_MODE) {
      setJobs(prev => prev.map(j => j.status === 'pending' ? { ...j, status: 'approved' } : j));
      toast({ title: "Đã duyệt toàn bộ", description: `Đã duyệt ${pendingJobs.length}/${pendingJobs.length} tin tuyển dụng` });
      return;
    }
    try {
      const token = await getAccessToken();
  ...
  const handleApprove = async (jobId: number) => {
    if (DEMO_MODE) {
      ...
      toast({ title: "Đã duyệt", description: "Tin tuyển dụng demo đã được phê duyệt" });
      return;
    }
    try {
      const token = await getAccessToken();
  ...
  const handleReject = async (jobId: number) => {
    if (DEMO_MODE) {
      ...
      toast({ title: "Đã từ chối", description: "Tin tuyển dụng demo đã bị từ chối" });
      return;
    }
    try {
      const token = await getAccessToken();
  ...
  const handleDeleteJob = async () => {
    ...
    if (DEMO_MODE) {
      ...
      toast({ title: 'Đã ẩn bài đăng', description: 'Bài đăng demo đã bị ẩn' });
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Ẩn bài đăng?</DialogTitle>
            <DialogDescription>
