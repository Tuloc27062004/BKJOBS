...
const Jobs = () => {
  ...
  useEffect(() => {
    ...
    const fetchDistricts = async () => {
      if (DEMO_MODE) {
        setDistricts(Array.from(((DEMO_DISTRICTS as Record<string, readonly District[]>)[province] || [])));
        return;
      }
  ...
  useEffect(() => {
    ...
    const fetchWards = async () => {
      if (DEMO_MODE) {
        setWards(Array.from(((DEMO_WARDS as Record<string, readonly Ward[]>)[district] || [])));
        return;
      }
  ...
  const fetchJobs = async () => {
    setLoading(true);
    if (DEMO_MODE) {
      const approvedJobs = (DEMO_JOB_FORMS as JobForm[]).filter((j: JobForm) => j.status === 'approved');
